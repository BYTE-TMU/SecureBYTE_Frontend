  ...
  const saveToBackendWithRetry = async (fileId, fileName, filePath, content, retryCount = 0) => {
    ...
    try {
      ...
      if (isRateLimited && shouldRetry) {
        ...
        return saveToBackendWithRetry(fileId, fileName, filePath, content, retryCount + 1);
  ...
  const handleSaveAllFiles = async () => {
    ...
    if (successCount > 0 && failCount === 0) {
      ...
      if (rateLimitedCount > 0) {
        toast.error(`Failed to save ${failCount} file(s) - server is busy. Please try again in a moment.`);
      } else {
